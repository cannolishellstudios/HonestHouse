...
export default function WhatIfScreen() {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: '#fff' }} contentContainerStyle={{ paddingBottom: 100 }}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            <View>
              <View style={{ flexDirection: 'row', backgroundColor: colors.surface }}>
                ...
                {sc.downs.map(d => <View key={`hdr-${d}`} style={{ width: 68, padding: 8, alignItems: 'center' }}><Text style={{ color: colors.textMed, fontSize: 9, fontWeight: '800', textTransform: 'uppercase' }}>{Math.round(d * 100)}%</Text></View>)}
              </View>
              {sc.prices.map((p, pi) => (
                <View key={`row-${pi}`} style={{ flexDirection: 'row', borderTopWidth: 1, borderTopColor: colors.surface }}>
                  ...
                  {sc.downs.map((dp, di) => {
                    ...
                    const k = `${pi}-${di}`, sl2 = sel === k, cl = ok ? colors.green : tight ? colors.yellow : colors.red, bg = ok ? colors.greenBg : tight ? colors.yellowBg : colors.redBg;
                    return <TouchableOpacity key={`cell-${pi}-${di}`} onPress={() => sS(sl2 ? null : k)} activeOpacity={0.7} style={{ width: 68, padding: 2, alignItems: 'center' }}>
      ...
      {/* SELECTED DETAIL */}
      {sel && (() => {
        const [pi, di] = sel.split('-'); const d = gD(parseInt(pi), sc.downs[parseInt(di)]);
        return <View style={{ paddingHorizontal: 16, marginBottom: 12 }}>
          <ChunkyCard color={d.ok ? colors.green : colors.red} shadowColor={d.ok ? colors.greenDark : colors.redDark}>
            <Text style={{ color: colors.text, fontSize: 15, fontWeight: '900', marginBottom: 10 }}>{fK(d.p)} · {Math.round(sc.downs[parseInt(di)] * 100)}% down</Text>
